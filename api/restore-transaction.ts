...
import type { BudgetData, Transaction } from "../src/types";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const existing = data.transactions.find((t) => t.id === tx.id);
    if (existing) {
    ...
    data.funds[tx.fund] = Number((data.funds[tx.fund] + tx.amount).toFixed(2));
    ...
    const putResp = await fetch(`${GH_API}/repos/${OWNER}/${REPO}/contents/${PATH}`, {
      ...
      body: JSON.stringify({
        message: `Restore transaction: ${tx.id}`,
        content: newContent,
        sha: file.sha,
      }),
    });
